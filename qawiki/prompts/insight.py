...
SEGMENT_SUMMARY_PROMPT = """You are a World-Class reasoning analysis expert. A multimodal agent system uses tool-based visual reasoning to solve the given problem. The agent may have been provided with some experiences and skill guidance. Please summarize the following trajectory (also called rollout) step-by-step:
...
CROSS_SOURCE_CRITIQUE = """You are a reasoning analysis expert. Review these problem-solving attempts and extract practical lessons that could help future attempts.
...
INSIGHT_MERGE_PROMPT = """You are an insight library management expert. Merge the following experiences into a single, comprehensive experience.
...
INSIGHT_REFINE_PROMPT = """You are an experience library curator. The library has grown through multiple batches and may contain redundancy. Perform a global refinement pass.
