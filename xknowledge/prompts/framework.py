...
GENERATE_FRAMEWORK_PROMPT = """You are a skilled AI agent architect. Analyze the trajectory and extract a reusable Standard Operating Procedure (SOP).
...
MERGE_FRAMEWORK_PROMPT = """You are a knowledge architect. Your job is to maintain a single, unified skill document that grows wiser with each new case.
...
- **Is this part better?** 鈫?Rewrite the existing version
- **Is this part redundant or too specific?** 鈫?Delete it
- **Is this part complementary?** 鈫?Merge into a more general form
- **Is this part genuinely different?** 鈫?Add as a variant workflow (but consolidate if possible)
...
FRAMEWORK_REFINE_PROMPT = """You are a skill document architect. Refine the SKILL.md to remove redundancy, generalize specific cases, and improve structure.
...
4. **Format Optimization**:
   ...
   - Organize content from general principles 鈫?specific techniques
