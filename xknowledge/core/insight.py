"""Insight library management.
Adapted from XSkill eval/exskill/experience_manager.py
...
from .llm import ExperienceLLM
from .retriever import KnowledgeRetriever

from ..prompts.insight import INSIGHT_REFINE_PROMPT, INSIGHT_MERGE_PROMPT
...
class InsightMemoryProvider:
    """
    Manages experiences with embedding-based similarity search and merge operations.
    Wraps KnowledgeRetriever for embedding functionality.
    """
    
    def __init__(self, experiences: Dict[str, str], retriever: Optional[KnowledgeRetriever] = None):
        """
        Initialize the provider with existing experiences.
        
        Args:
            experiences: Dictionary mapping experience IDs to experience text
            retriever: Optional KnowledgeRetriever instance (will create one if not provided)
        """
    ...
    def _get_retriever(self) -> KnowledgeRetriever:
        ...
        if self._retriever is None:
            ...
            self._retriever = KnowledgeRetriever(
                ...
                embedding_model=os.environ.get("INSIGHT_EMBEDDING_MODEL", "text-embedding-3-small"),
                embedding_api_key=os.environ.get("INSIGHT_EMBEDDING_API_KEY") or os.environ.get("OPENAI_API_KEY"),
                embedding_endpoint=os.environ.get("INSIGHT_EMBEDDING_ENDPOINT") or os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1"),
    ...
    def add_insight(self, exp_id: str, text: str):
        """Add a new experience."""
        self.experiences[exp_id] = text
    
    def remove_insight(self, exp_id: str):
        """Remove an experience."""
        if exp_id in self.experiences:
            del self.experiences[exp_id]
    
    def modify_insight(self, exp_id: str, new_text: str):
...
def _llm_merge_insights(
    ...
    prompt = INSIGHT_MERGE_PROMPT.format(experiences_text=experiences_text)
...
def _process_add_insight(
    provider: InsightMemoryProvider,
    new_text: str,
    llm: ExperienceLLM,
    similarity_threshold: float = SIMILARITY_THRESHOLD,
    next_id: int = 0
    ...
    Args:
        provider: InsightMemoryProvider instance
    ...
    if new_emb.size == 0:
        ...
        exp_id = f"E{next_id}"
        provider.add_insight(exp_id, new_text)
        return exp_id, False
    ...
    merged_text = _llm_merge_insights(all_candidates, llm)
    
    if merged_text:
        # Remove existing experiences that were merged
        for exp_id, _, _ in candidates:
            provider.remove_insight(exp_id)
        
        # Add merged experience
        exp_id = f"E{next_id}"
        provider.add_insight(exp_id, merged_text)
...
def batch_merge_insights(
    ...
    Flow:
    - add operations: Check similarity 鈫?if high, directly merge with similar experiences
    ...
    Args:
        ...
        similarity_threshold: Threshold for triggering merge
        retriever: Optional KnowledgeRetriever instance for embedding (will create if needed)
    ...
    if not updates:
        ...
        if isinstance(experience_limit, int) and experience_limit > 0 and len(existing) > experience_limit:
            try:
                capped = _reduce_insights_to_limit(existing, experience_limit, llm, debug_dir, similarity_threshold=similarity_threshold, retriever=retriever)
    ...
    provider = InsightMemoryProvider(existing, retriever=retriever)
    ...
    for operation in updates:
        ...
        if option == "modify":
            ...
            if modified_from and modified_from in provider.experiences:
                ...
                max_id += 1
                exp_id, was_merged = _process_add_insight(provider, experience, llm, similarity_threshold, max_id)
                add_count += 1
                if was_merged:
                    merge_count += 1
        elif option == "add":
    ...
    print(f"  - Batch update: {len(updates)} updates 鈫?{len(to_modify)} modifies, {add_count} adds ({merge_count} merged)")
    ...
    for operation in to_modify:
        ...
        if modified_from and new_text:
            provider.modify_insight(modified_from, new_text)
    ...
    if isinstance(experience_limit, int) and experience_limit > 0 and len(final_experiences) > experience_limit:
        ...
        try:
            reduced_experiences = _reduce_insights_to_limit(final_experiences, experience_limit, llm, debug_dir, similarity_threshold=similarity_threshold, retriever=retriever)
...
batch_merge = batch_merge_insights


def _reduce_insights_to_limit(
    ...
    provider = InsightMemoryProvider(current, retriever=retriever)
            ...
            for pair_idx, (exp_id1, exp_id2, sim_score) in enumerate(similar_pairs[:pairs_to_merge]):
                ...
                try:
                    ...
                    if merged_text:
                        ...
                        provider.remove_insight(exp_id1)
                        provider.remove_insight(exp_id2)
                        
                        # Add merged experience
                        new_id = f"E{_max_exp_id(current) + 1}"
                        provider.add_insight(new_id, merged_text)
...
def refine_insight_library(
    ...
    prompt = INSIGHT_REFINE_PROMPT.format(
        ...
        if option == "merge":
            ...
            if merged_from and len(merged_from) >= 2 and merged_text:
                ...
                if len(valid_sources) >= 2:
                    ...
                    merge_count += 1
                    print(f"    - Merged {valid_sources} 鈫?{new_id}")
    ...
    print(f"  [Refine] Complete: {len(experiences)} 鈫?{len(final)} ({merge_count} merges, {delete_count} deletes)")
    
    return final


refine_experience_library = refine_insight_library
