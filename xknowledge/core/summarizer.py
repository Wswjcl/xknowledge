"""Trajectory summarization for knowledge distillation.
Adapted from XSkill eval/exskill/experience_summarizer.py
...
def summarize_rollouts(traj_paths: Union[str, List[str]], llm: ExperienceLLM, sample_dir: Optional[str] = None) -> dict:
    ...
    if not all_images:
        ...
        if generate_image_captions is not None:
    ...
    prompt = SEGMENT_SUMMARY_PROMPT.format(trajectory=process_text)
