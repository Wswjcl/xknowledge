...
Provides retrieval-based insight injection by selecting relevant insights
based on query embedding similarity, rather than injecting all insights.

Adapted from XSkill eval/exskill/experience_retriever.py
...
TOP_K_RETRIEVE = 5  # Default number of top experiences to retrieve
...
# --------- Prompts ---------
from ..prompts.query import TASK_DECOMPOSITION_PROMPT, INSIGHT_REWRITE_PROMPT
...
def _build_multimodal_user_content(prompt: str, images: List[Any]) -> List[dict]:
    ...
    for img in images:
        if isinstance(img, Image.Image):
            content.append({
                "type": "image_url",
                "image_url": {"url": "data:image/jpeg;base64," + image_to_base64(img)}
            })
    return content
...
class KnowledgeRetriever:
    ...
    def __init__(
        ...
        insight_library_path: Optional[str] = None,  # Path to experience library JSON file
    ):
        """
        ...
        Args:
            ...
            embedding_api_key: API key for embedding service (defaults to INSIGHT_EMBEDDING_API_KEY or OPENAI_API_KEY)
            embedding_endpoint: API endpoint for embedding service (defaults to INSIGHT_EMBEDDING_ENDPOINT or OPENAI_BASE_URL)
            ...
            insight_library_path: Path to experience library JSON file (used to name cache subfolder)
        """
        ...
        self.insight_library_path = insight_library_path
        ...
        self.embedding_api_key = (
            embedding_api_key 
            or os.environ.get("INSIGHT_EMBEDDING_API_KEY")
        ...
        self.embedding_endpoint = (
            embedding_endpoint
            or os.environ.get("INSIGHT_EMBEDDING_ENDPOINT")
        ...
        if not self.embedding_api_key:
            raise ValueError(
                "Embedding API key is required. Set INSIGHT_EMBEDDING_API_KEY "
        ...
        if self.enable_cache:
            ...
            if self.insight_library_path:
                # Extract filename without extension from library path
                library_filename = os.path.basename(self.insight_library_path)
    ...
    def _decompose_task_llm(self, task_description: str, images: Optional[List[Any]] = None) -> List[Dict[str, str]]:
        ...
        if not self.llm_client:
            raise ValueError("LLM client is required for task decomposition. Pass llm_client to KnowledgeRetriever.__init__")
    ...
    def retrieve_with_decomposition(
        ...
        if not self.llm_client:
            raise ValueError(
                ...
                "Pass llm_client to KnowledgeRetriever.__init__ or use retrieve() instead"
            )
...
def rewrite_insights_for_task(
    ...
    try:
        ...
        prompt = INSIGHT_REWRITE_PROMPT.format(
...
rewrite_experiences_for_task = rewrite_insights_for_task
ExperienceRetriever = KnowledgeRetriever
